...
export default function LoveNotesWall() {
  ...
  // Load Surya's custom notes (editable via edit panel) + Sadhana's saved replies
  useEffect(() => {
    Promise.all([
      dbGet("wall_surya_notes", DEFAULT_SURYA_NOTES),
      dbGet("wall_notes", []),
    ]).then(([suryaRaw, sadhanaRaw]) => {
      const suryaArr = Array.isArray(suryaRaw) && suryaRaw.length
        ? suryaRaw
        : DEFAULT_SURYA_NOTES;
      const suryaNotes = suryaArr.map((item, i) => ({
        id:    `surya_${i}`,
        color: NOTE_COLORS[i % NOTE_COLORS.length],
        rot:   NOTE_ROTS[i % NOTE_ROTS.length],
        from:  "Surya",
        text:  typeof item === "string" ? item : item.text || "",
        emoji: NOTE_EMOJIS[i % NOTE_EMOJIS.length],
      }));
      const sadhana = Array.isArray(sadhanaRaw) ? sadhanaRaw : [];
      setNotes([...suryaNotes, ...sadhana]);
      setLoading(false);
    });
